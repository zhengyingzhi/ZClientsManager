...
{
	if (m_Capacity <= (m_PrePaddingSize + m_Length))
		return 0;
	return m_Capacity - m_PrePaddingSize - m_Length;
}
...
uint32_t ZFixApi::PrePaddingSize() const
{
	return m_PrePaddingSize;
}

void ZFixApi::SetItem(uint32_t aID, char aValue)
{
	m_Length += snprintf(m_pBuff + m_Length, AvailSize(), "%d=%c%d", aID, aValue, ZFIX_DELIMITER);
}

void ZFixApi::SetItem(uint32_t aID, const uint16_t aValue)
{
	m_Length += snprintf(m_pBuff + m_Length, AvailSize(), "%d=%d%d", aID, aValue, ZFIX_DELIMITER);
}
...
{
	m_Length += snprintf(m_pBuff + m_Length, AvailSize(), "%d=%u%d", aID, aValue, ZFIX_DELIMITER);
}

void ZFixApi::SetItem(uint32_t aID, int64_t aValue)
{
	m_Length += snprintf(m_pBuff + m_Length, AvailSize(), "%d=%lld%d", aID, aValue, ZFIX_DELIMITER);
}
...
void ZFixApi::SetItem(uint32_t aID, const char* apValue, int aLength)
{
	...
	m_Length += snprintf(m_pBuff + m_Length, AvailSize(), "%d=", aID);
...
{
	if (m_Length <= m_PrePaddingSize) {
		return -1;
	}

	int lArrSize = 128;
	char* lpArr[128] = {};
	lArrSize = str_delimiter(m_pBuff + m_PrePaddingSize, lpArr, lArrSize, ZFIX_DELIMITER);
	if (lArrSize == 0) {
		return -2;
	}
